#ifndef PATTERN_POLAR_TTA_SCAN_SPC_HPP_
#define PATTERN_POLAR_TTA_SCAN_SPC_HPP_
...
namespace tools
{
class Pattern_polar_TTA_SCAN_spc : public Pattern_polar_spc
{
protected:
	Pattern_polar_TTA_SCAN_spc(const int &N, const Binary_node<Pattern_polar_i>* node,
...
public:
	Pattern_polar_TTA_SCAN_spc(const int min_level = 2, const int max_level = -1)
	: Pattern_polar_spc(min_level, max_level) {}

	virtual ~Pattern_polar_TTA_SCAN_spc() {}

	virtual Pattern_polar_TTA_SCAN_spc* alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const
	{
		if (node == nullptr)
			return new Pattern_polar_TTA_SCAN_spc(this->min_level, this->max_level);
		else
			return new Pattern_polar_TTA_SCAN_spc(N, node, this->min_level, this->max_level);
	}
...
#endif /* PATTERN_POLAR_TTA_SCAN_SPC_HPP_ */
