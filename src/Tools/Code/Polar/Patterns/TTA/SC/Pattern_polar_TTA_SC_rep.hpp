#ifndef PATTERN_POLAR_TTA_SC_REP_HPP
#define PATTERN_POLAR_TTA_SC_REP_HPP
...
namespace tools
{
class Pattern_polar_TTA_SC_rep : public Pattern_polar_rep
{
protected:
	Pattern_polar_TTA_SC_rep(const int &N, const Binary_node<Pattern_polar_i>* node,
...
public:
	Pattern_polar_TTA_SC_rep(const int min_level = 1, const int max_level = -1)
	: Pattern_polar_rep(min_level, max_level) {}

	virtual ~Pattern_polar_TTA_SC_rep() {}

	virtual Pattern_polar_TTA_SC_rep* alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const
	{
		if (node == nullptr)
			return new Pattern_polar_TTA_SC_rep(this->min_level, this->max_level);
		else
			return new Pattern_polar_TTA_SC_rep(N, node, this->min_level, this->max_level);
	}
...
#endif /* PATTERN_POLAR_TTA_SC_REP_HPP */
