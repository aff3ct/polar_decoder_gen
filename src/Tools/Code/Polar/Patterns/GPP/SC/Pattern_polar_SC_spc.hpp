...
public:
	...
	virtual Pattern_polar_SC_spc* alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const
	{
		if (node == nullptr)
			return new Pattern_polar_SC_spc(this->min_level, this->max_level);
		else
