...
public:
	...
	virtual Pattern_polar_SC_rep* alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const
	{
		if (node == nullptr)
			return new Pattern_polar_SC_rep(this->min_level, this->max_level);
		else
